...
class SqliteDbDriver(BaseDbDriver):
    ...
    def script_rep(self, user_id, script_id, date_from, date_to):
        ...
        self._cursor.execute(
            ...
            "            then round(cast(sub.obj_cnt * 100 as real) "
            "                 / sub.check_obj_cnt, 2) "
            "        else 0 end as perc_obj_cnt, "
            "    sub.tr_obj_cnt, "
            "    case "
            "        when sub.obj_cnt > 0 "
            "            then round(cast(sub.tr_obj_cnt * 100 as real) "
            "                 / sub.obj_cnt, 2) "
            "        else 0 end as perc_tr_obj_cnt, "
            "    sub.wr_obj_cnt, "
            "    case "
            "        when sub.obj_cnt > 0 "
            "            then round(cast(sub.wr_obj_cnt * 100 as real) "
            "                 / sub.obj_cnt, 2) "
            "        else 0 end as perc_wr_obj_cnt, "
            "    sub.er_obj_cnt, "
            "    case "
            "        when sub.obj_cnt > 0 "
            "            then round(cast(sub.er_obj_cnt * 100 as real) "
            "                 / sub.obj_cnt, 2) "
